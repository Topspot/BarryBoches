...
if (!class_exists("cForm")) {
  ...
  PRIMARY KEY (`lead_id`)
        ...
        public function get_cForm() {
            ...
            <div class="cForm_widget">            
                <form action = "" method = "post" name = "cform" id = "cform">
                    ...
                    <p><button  class="btn <?php $this->EmptyFieldCheck("btn-contact", $cForm_options['btnclass']) ?>" id = "cformsubmit" ><span>GET HELP NOW</span></button> </p>
                </form>
            </div>
            <?php
        }
        ...
        function save_send_mail($formvalues) {
            ...
            include 'mail_classess/class_phpmailer.php';
            $mails = new PHPMailer;
            include 'mail_classess/smtp_setting.php';
            ...
            $mails->From = 'support@speakeasymarketinginc.com';
            $mails->FromName = $site_url;

            for ($i = 0; $i < sizeof($email_array); $i++) {
                $mails->addAddress($email_array[$i]);
            }

            $mails->addReplyTo($formvalues['cformemail']);

            $mails->WordWrap = 50;
            $mails->isHTML(true);

            $mails->Subject = $subject;
            $mails->Body = $email_temp;
            ...
            if ($status) {
                ...
                $lead_server_data['lead_source'] = 'Contact Form'; /* Phone call */
                $lead_server_data['client_name'] = $admin_name;
                $lead_server_data['website'] = $site_url;
                $lead_server_data['inc_phone'] = $formvalues['cformphone']; /* Caller id */
                $lead_server_data['call_time'] = date('h:i A'); /* Time of lead */
                $lead_server_data['call_duration'] = '';
                $lead_server_data['lead_name'] = $formvalues['cformname']; /* Caller name */
                $lead_server_data['lead_email'] = $formvalues['cformemail']; /* Caller Email */
                $this->SendDataToWebsite($lead_server_data);
        ...
        function SendDataToWebsite($lead_data2) {
            $curl_options = array(
