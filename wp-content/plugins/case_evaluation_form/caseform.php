    ...
    <!--<form action = "<?php bloginfo('url') ?>?action=caseform&redirect_url=case-thank-you" method = "post" name = "caseform" id = "caseform">-->
            <div class="caseForm_widget">            
                ...
                <input type="text" class = "cfom-text" id="cfom-text" name="caseformzip"></span>
                     ...
                     <p>
                         <?php
                        $this->EmptyFieldCheck("Phone", $caseForm_options['phonelabel']);
                     ...
                     <p><?php
                        $this->EmptyFieldCheck("Date of Appointment", $caseForm_options['dateofapplabel']);
                        $this->get_required('dateofapp')
                        ?> <br>
                        <span>
                <input type="text" class = "cfom-text" id="cfom-text" name = "caseformdateofapp"></span>
                    </p>
                     <p><?php
                        $this->EmptyFieldCheck("How did you find us?", $caseForm_options['findUslabel']);
                        $this->get_required('findUs')
                        ?> <br>
                        <span>
                    <input type="text" class = "cfom-text" id="cfom-text" name = "caseformfindUs"></span>
                    </p>
                    <p><?php
                        $this->EmptyFieldCheck("Statement of Case", $caseForm_options['casestatementlabel']);
                        $this->get_required('casestatement')
                        ?> <br>
                        <span>
                            <textarea rows = "4" cols = "20" class="caseform-textarea" id = "caseformmessage" name = "caseformcasestatement"></textarea>
                        </span>
                    </p>
                    ...
                    <p><button class="btn-yellow" class="<?php $this->EmptyFieldCheck("btn-case_evaluation", $caseForm_options['btnclass']) ?>" id = "caseformsubmit" > <i class="icon-share"></i> SUBMIT</button> </p>
                </form>
            </div>
            <?php
        }
        ...
        function get_form_case_parameters() {
            ...
            if (isset($_POST['caseform_type'])) {
                ...
                $caseformValue['caseformdateofapp'] = $_POST['caseformdateofapp'];
                $caseformValue['caseformfindUs'] = $_POST['caseformfindUs'];
                
                $caseformValue['caseformcasestatement'] = $_POST['caseformcasestatement'];
...
//                    print_r($caseformValue);exit;
        ...
        function save_send_mail($formvalues) {
            ...
            $email_temp = @str_replace("[findus]", $formvalues['caseformfindUs'], $email_temp);
            $email_temp = @str_replace("[dateofapp]", $formvalues['caseformdateofapp'], $email_temp);
            $email_temp = @str_replace("[statement]", $formvalues['caseformcasestatement'], $email_temp);
            $email_temp = @str_replace("[comments]", $formvalues['caseformcomments'], $email_temp);
            
            //print_r($formvalues['caseformemail']);exit;
            $headers = 'MIME-Version: 1.0' . "\r\n";
            $headers.= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
            $headers.= "From: $site_url <" . $formvalues['caseformemail'] . ">" . "\r\n";
            $headers.= 'Reply-To: ' . 'seolawyers2012@gmail.com'. "\r\n";
